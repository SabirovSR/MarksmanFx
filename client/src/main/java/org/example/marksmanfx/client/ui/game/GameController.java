 ...
 *   — поддержание трёхфазной стейт-машины кнопки паузы (Fix 7)
 *   — отображение начального списка игроков комнаты (Fix 5)
 *   — техническую победу при дисконнекте соперника (Fix 2)
 *   — систему реванша после окончания матча (Fix 5)
 */
public final class GameController implements MessageListener {
    ...
    /** Имя победителя (null если технический win). */
     ...
     *                        в оверлее ожидания немедленно (Fix 5).
     */
    ...
    private void renderFrame(long now) {
        ...
        if (gameOver) {
            ...
            // Fix 2: Выбираем нужный вид оверлея — технический или обычный
        ...
        // Fix 6: Рисуем баннер запроса паузы на основе PauseStateMessage,
    ...
    public void onMessage(ServerMessage message) {
        switch (message) {
            ...
            case TechnicalWinMessage       m -> onTechnicalWin(m);        // Fix 2
            case RematchOfferMessage       m -> onRematchOffer(m);        // Fix 5
            case PauseStateMessage         m -> onPauseState(m);
            case RoomUpdatedMessage        m -> onRoomUpdated(m);         // Fix 5
    ...
    /**
     * Fix 2: Техническая победа — соперник покинул матч.
    ...
    /**
     * Fix 5: Обновляем индикатор реванша — кто уже проголосовал.
    ...
    /**
     * Fix 7: Получаем PauseStateMessage и обновляем клиентскую фазу.
    ...
    /** Fix 5: Список игроков в комнате изменился — обновляем оверлей ожидания. */
    ...
    private void onReady() {
        switch (clientPhase) {
            ...
            case FINISHED -> {
                // Fix 5: Предлагаем реванш — блокируем кнопку после нажатия
                if (!rematchVoted) {
                    rematchVoted = true;
    ...
    /**
     * Fix 7: Контекстная кнопка паузы.
    ...
    private void releaseCharge() {
        ...
        chargeStartNanos = -1;
        // Fix 3 (клиентская часть): отправляем команду выстрела,
    ...
    private void updateButtons() {
        switch (clientPhase) {
            ...
            case FINISHED -> {
                // Fix 5: два варианта действий после матча
                pauseButton.setDisable(true);
                pauseButton.setText("Пауза [P]");
                resetPauseButtonStyle();
    ...
    /** Fix 5: Строим список игроков в комнате для оверлея ожидания. */
